...
* If you plan on using  uC/OS-II  in a commercial product you need to contact Micri嚙練 to properly license 
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    char msg[200];
                    sprintf(msg, "%lu Preempted from %u to %u\n", OSTime, OSPrioCur, OSPrioHighRdy);
                    puts(msg);
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
                char msg[200];
                sprintf(msg, "%lu Completed from %u to %u\n", OSTime, OSPrioCur, OSPrioHighRdy);
                puts(msg);
#endif
...
static  void  OS_SchedNew (void)
{
    ...
    while(ptcb != 0) {
        // When your state is ready but the task might be in a delay situation(that means the task is finish)
        // Therefore, you have to make sure your state is READY AND NO DELAY

        if ( (ptcb->OSTCBPrio == 1 || ptcb->OSTCBPrio == 2) && ptcb->OSTCBStat == OS_STAT_RDY && ptcb->OSTCBDly == 0){
            if (ptcb->period <= min) {
                min = ptcb->period;
